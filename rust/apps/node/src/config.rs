...
impl Config {
    pub fn from_environment() -> Result<Self> {
        let addr: SocketAddr = env::var("NODE_ADDR")
            .context("failed find $NODE_ADDR")?.parse()
            .context("$NODE_ADDR is not ip address")?;

        let write_timeout: u64 = env::var("NODE_WRITE_TIMEOUT").context("failed find $NODE_WRITE_TIMEOUT")?.parse().context("$NODE_WRITE_TIMEOUT is not u64")?;
        let read_timeout: u64 = env::var("NODE_READ_TIMEOUT").context("failed find $NODE_READ_TIMEOUT")?.parse().context("$NODE_READ_TIMEOUT is not u64")?;
